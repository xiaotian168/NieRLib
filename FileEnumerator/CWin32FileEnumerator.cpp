...
{
	return new CWin32FileEnumerator();
}

CWin32FileEnumerator::CWin32FileEnumerator()
{
...
bool CWin32FileEnumerator::EnumAllFileW(const wchar_t * pszDirectory, const wchar_t * pszFileExt, const bool bEnumSubDir, std::list<IFileEnumResult *> & ResultList)
{
	bool bRet = false;
			...
			do 
			{
				...
				else
				{
					...
					if (0 == _wcsicmp(L"*.*", pszFileExt) || 0 == _wcsicmp(szResultExt, pszFileExt))
					{
						...
						auto pFindResult = CWin32FileEnumResult::Make(szResultPath, Win32FindData.dwFileAttributes);
						if (pFindResult)
						{
...
bool CWin32FileEnumerator::EnumAllDirectoryW(const wchar_t * pszDirectory, const bool bEnumSubDir, std::list<IFileEnumResult *> & ResultList)
{
	bool bRet = false;
	WIN32_FIND_DATAW Win32FindData = { 0 };
	wchar_t szFindPath[MaxPathSize] = { 0 };
